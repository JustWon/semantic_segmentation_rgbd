...
class FCN_RGBHHA_mask2(nn.Module):

    def __init__(self, n_classes=21, learned_billinear=False):
        super(FCN_RGBHHA_mask2, self).__init__()
        ...
        fm = 512
        self.masked_conv4 = nn.Sequential(
        ...
        fm = 256
        self.masked_conv3 = nn.Sequential(
            MaskedConv2d('A', fm, fm, 7, 1, 3, bias=False), nn.BatchNorm2d(fm), nn.ReLU(True),
            MaskedConv2d('B', fm, fm, 7, 1, 3, bias=False), nn.BatchNorm2d(fm), nn.ReLU(True),
        )
    ...
    def forward(self, color, depth):
        ...
        conv4 = self.masked_conv4(conv4)
        score_pool4 = self.score_pool4(conv4)
        conv3 = self.masked_conv3(conv3)
        ...
        depth_conv4 = self.masked_conv4(depth_conv4)
        depth_score_pool4 = self.score_pool4(depth_conv4)
        depth_conv3 = self.masked_conv3(depth_conv3)
